...
router.get('/', asyncHandler(async (req, res) => {
  ...
  res.render("articles/index", { articles: articles, title: "Sequelize-It!" }); //can just put single articles as they match
}));
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  res.redirect("/articles/" + article.id); //appends the req.body id to url
  } catch (error) {
...
router.get("/:id", asyncHandler(async (req, res) => {
  const article = await Article.findByPk(req.params.id); //this id was initiatec under POST create article
  if(article) {
    res.render("articles/show", { article: article, title: article.title }); //can do shorthand for above of jusr article, title disp;ays in tab
  } else {
    res.sendStatus(404);
  }
}));
